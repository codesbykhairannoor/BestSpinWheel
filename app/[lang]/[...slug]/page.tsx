...
export async function generateMetadata({ params }: { params: { lang: string, slug: string[] } }): Promise<Metadata> {
  const intent = params.slug[0] || 'random';
  ...
  return {
    ...
    description: `The ultimate ${formattedIntent.toLowerCase()} tool. Free, fast, and secure. Use this generator for your needs in ${params.lang.toUpperCase()}.`,
    alternates: {
      canonical: `https://randompickerwheel.com/${params.lang}/${intent}`
    }
  };
}

export default function PSEOPage({ params }: { params: { lang: string, slug: string[] } }) {
  return (
    <>
      <div className="order-last opacity-0 h-0 overflow-hidden pointer-events-none -z-50 text-[1px] leading-[1px]">
        ...
        <h1>{params.slug.join(' ').replace(/-/g, ' ')} Random Picker</h1>
        <p>This is the dedicated page for {params.slug.join(' ').replace(/-/g, ' ')} in language {params.lang}. 
